...
login_manager.login_view = 'login'  # куда редиректить незалогиненных

@login_manager.user_loader
def load_user(user_id):
    return User.query.get(int(user_id))

# ✅ 4. Добавляем маршрут index (он нужен для url_for('index'))
@app.route('/')
def index():
    return redirect(url_for('dashboard'))  # или render_template('index.html')
